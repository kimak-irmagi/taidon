...
3. Существующие СУБД плохо поддерживают массовые параллельные экземпляры: снапшоты тяжёлые, изоляция дорогая.
...
25. Один пользователь = один экземпляр.
...
32. **Масштабируемость обучения** — сотни параллельных экземпляров без дублирования данных.
...
37. Не разрозненные экземпляры для каждого пользователя, а **оптимальное хранение версионных состояний**.
