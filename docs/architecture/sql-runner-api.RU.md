...
- Биндиться к песочнице через env-manager и соблюдать ресурсные/TTL политики.
...
- Sandbox TTL (env-manager) -> гасит idle/долго живущие песочницы.
...
- **Sandbox TTL**: enforced env-manager; idle песочницы могут быть убраны, что требует нового bind на следующий run.
...
- Метрики: cache hit ratio, planning latency, sandbox bind latency, head execution latency, rows/bytes streamed, cancellations, timeouts.
...
- **Scaling**: autoscaling для runner и cache builders; warm pools для песочниц.
