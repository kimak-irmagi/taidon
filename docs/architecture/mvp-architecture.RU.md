...
- Начальный remote profile management вводит организации как ownership
  container для зарегистрированного пользователя. Полная GitHub-подобная
  иерархия и organization-wide authorization для экземпляров остаются позже.
- Секреты (Git токены) хранятся в k8s Secrets; доступ сначала ограничен
  границами экземпляра/аккаунта.
...
- `user-profile`: учетные записи, external identities, организации,
  memberships, квоты/лимиты (без биллинга).
...
- **Метаданные платформы**: отдельный control-plane metadata store для
  пользователей, external identities, организаций, memberships, экземпляров,
  описаний снапшотов, аудита. Конкретная технология хранения не выбирается
  client slice.
...
- Invitations, управление memberships, роли кроме первого admin и сложный
  organization RBAC - позже.
