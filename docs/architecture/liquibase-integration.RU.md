...
Вычислить хеши для максимально возможного числа ближайших шагов *без их выполнения*, затем перемотаться по кэшу до последнего доступного состояния.
...
Во время перемотки песочница не создается.
...
Когда перемотка останавливается (cache miss или конец горизонта), Taidon материализует песочницу из последнего известного `base_id`.
...
### 7.4 Переключение песочницы при cache hit

Если для следующих шагов обнаружен cache hit:

- прекратить использование текущей песочницы
- создать новую песочницу из кэшированного State
...
При ошибке Taidon может сохранить snapshot отказавшей песочницы для расследования.
...
### 9.3 Идентификаторы для трекинга

Идентичность changeset (author/id/path) фиксируется для:

- диагностики
- UX отображения
- обнаружения plan drift

Она не требуется в cache key, если есть `block_hash` и `base_state_id`.
...
- Каждая песочница изолирована; запуск Liquibase не должен шарить одну DB-инстанцию.
