...
Goal: launch a minimal viable platform for SQL learning scenarios with isolated instances, deployed on-prem in Kubernetes (at minimum, minikube for local debugging).
...
- **SQL session run**: frontend -> gateway -> sql-runner. `sql-runner` computes tail hash, requests snapshot from `snapshot-cache`. If found, `env-manager` boots instance from snapshot and head is executed. If not, tail is replayed, the layer is stored in cache, then head is executed. Results/errors return to frontend.
- **Instance management**: create/delete/restart by user request or TTL; `env-manager` applies ResourceQuota/LimitRange/NetworkPolicy and mounts snapshot volume.
...
- **Network boundaries**: gateway exposed externally; services are ClusterIP. NetworkPolicy restricts access to Postgres instances to `sql-runner` only.
- **Resources**: per-instance CPU/RAM limits; node affinity/taints for nodes with fast disks for snapshots.
...
- Core metrics: error rate (5xx/4xx), cache hit ratio (tail hash), instance startup latency and head execution latency, active instance count, CPU/RAM/IO usage on snapshot nodes, cache size/eviction rate.
- Alerts: low hit ratio, increasing instance startup time, PV saturation for snapshots, spikes in auth/Git errors.
- Grafana dashboards for requests/instances/cache.
...
- TTL policy for anonymous instances (resource control).
