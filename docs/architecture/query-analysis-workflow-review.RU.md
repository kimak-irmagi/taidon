...
### 2.1 Подготовить два инстанса БД (две "версии")
...
- Решите, сравниваете ли вы **warm-cache** или **cold-cache**.
  ...
  - cold-cache: перезапускать инстанс БД между прогонами
...
## 4. С sqlrs (минимальное вмешательство, единый prepare + run)

### 4.1 Базовая идея

С `sqlrs` **строка подключения к БД выводится из логической спецификации состояния**, а не из заранее поднятого инстанса.
...
### 4.2 Единый workflow prepare + run

Вместо явного создания инстансов или глобальных тегов, рекомендуемый workflow — это **одна команда**, которая:
...
- Создается эфемерная песочница из этого состояния.

- Обернутая команда получает обычное подключение к БД через env/DSN.

### 4.3 Альтернатива: явный DSN или экспорт окружения
...
- Используйте эфемерные sandboxes для измерений, чтобы избежать "загрязнения" между прогонами.
