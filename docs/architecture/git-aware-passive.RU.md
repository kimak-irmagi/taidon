...
- **Remote режим требует доступа к репозиторию.** Для `--ref` на удалённом раннере нужен server-side mirror (зеркало репозитория на стороне сервиса) или VCS-секреты; иначе CLI загружает исходники в `source storage` и передает `source_id` (см. `sql-runner-api.md`).
...
загрузить исходники в `source storage` и передать `source_id` (см. `sql-runner-api.md`).
