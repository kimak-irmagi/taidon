...
| ID   | Use case                                | Акторы         |
| ---- | --------------------------------------- | -------------- |
| UC-1 | Поднять изолированный экземпляр БД      | P1, P2, P3, P5 |
...
- Управление экземплярами и просмотр результатов
...
- Планирует операции жизненного цикла экземпляров
- Применяет квоты и TTL политики

##### Taidon Engine

- Базовая логика жизненного цикла экземпляров
...
- Триггеры запуска/обновления экземпляров по веткам/коммитам

##### Autoscaling Controller (team/cloud)

- Управляет пулами экземпляров и воркеров кэша (HPA/VPA/Cluster Autoscaler)
- Основан на метриках: очередь оркестрации, активные экземпляры, CPU/IO узлов хранилища, cache hit/latency
