...
- User: @evilguest
- Agent: Codex (GPT-5)
- Question: How should Windows local setup provision the Linux engine binary and
  btrfs storage for WSL2?
- Alternatives:
...
- Decision: Bundle Windows CLI + Windows engine + Linux engine (same CPU arch).
  `sqlrs init` copies the Linux engine into the WSL distro and manages btrfs
  volume setup (validate/init/re-init).
- Rationale: Keeps setup offline-friendly, minimizes user steps, and enables
  repeatable automation for WSL2 + btrfs.
