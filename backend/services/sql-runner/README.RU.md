...
Сервис детерминированного исполнения цепочек SQL. Вычисляет head/tail, ищет готовый снимок состояния в `snapshot-cache`, поднимает или переиспользует экземпляр через `env-manager`, выполняет head и возвращает результат вместе с метаданными слоя.
...
- поднимает/рестартует экземпляр PostgreSQL через `env-manager` и выполняет head;
...
4. `env-manager` поднимает экземпляр из найденного/созданного слоя либо из базового образа.
5. Executor подключается к PostgreSQL экземпляра, выполняет head, фиксирует ошибки/результат.
...
- Env binding - работа с `env-manager`, контроль TTL/лимитов экземпляров.
...
- Прогресс: статус (`pending` → `preparing` → `running` → `publishing` → `completed`/`failed`/`canceled`), этапы (парсинг, поиск снапшота, поднятие экземпляра, выполнение head), промежуточные логи.
...
- `POST /v1/jobs/{job_id}/cancel` — отменить выполнение и зачистить экземпляр.
...
- `env-manager` - создание/рестарт экземпляров, сетевые policy (PostgreSQL доступен только `sql-runner`).
- `audit-log` - запись действий и ошибок пользователя.
- `telemetry/exporter` - метрики Prometheus и трассировки.

## Наблюдаемость и безопасность

- Метрики: время холодного/тёплого старта экземпляра, длительность head, доля cache hit, ошибки подключения/SQL, прогресс заданий, доля отмен/таймаутов, загрузка GC.
...
- Сетевые границы: доступ к PostgreSQL экземплярам ограничен `sql-runner`; внешние вызовы идут только через `gateway`.
...
- Описать политику дедупликации/GC слоёв, TTL экземпляров и TTL заданий (с правилами премиум-планов).
