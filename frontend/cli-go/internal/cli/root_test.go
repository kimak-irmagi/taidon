...
func TestParseArgsCompositePrepareAliasRunAliasWithRefOptions(t *testing.T) {
	_, cmds, err := ParseArgs([]string{
		"prepare", "--ref", "HEAD~1", "--ref-mode", "blob", "chinook",
		"run", "smoke",
	})
	if err != nil {
		t.Fatalf("parse args: %v", err)
	...
	if got := strings.Join(cmds[0].Args, "|"); got != "--ref|HEAD~1|--ref-mode|blob|chinook" {
